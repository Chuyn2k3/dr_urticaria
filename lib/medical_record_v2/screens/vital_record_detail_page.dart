...
//   /// Giữ controller cho ghi chú theo vitalId -> tránh tạo mới mỗi build
...
//         // cập nhật nhưng giữ caret hợp lý (đưa về cuối để không "giật" con trỏ)
...
//             // ----- Chuẩn hoá & sort group ổn định -----
//             final all = List.of(state.groups)..sort(_compareGroups);
//             final totalSteps = all.length;
...
//               // (Tuỳ chọn) dọn các controller không còn dùng nữa
//               // final vitalIdsInGroup = items
//               //     .map<int>((it) => it.value.vitalIndicatorId as int)
//               //     .toSet();
//               // _noteControllers.removeWhere((k, _) => !vitalIdsInGroup.contains(k));
...
//                   // Chuẩn bị controller bền cho ghi chú
//                   final noteInitial =
//                       state.editedNotes[vitalId] ?? item.value.note ?? '';
//                   final noteController = _noteCtrl(vitalId, noteInitial);
...
//   // ===== PHÂN LOẠI GROUP ID =====
//   static const Set<int> _diagnosisIds = {25, 32, 35};
//   static const Set<int> _labIds = {24};
//   static const Set<int> _treatmentIds = {26};
...
//     final ta = _itemOrderTuple(a);
//     final tb = _itemOrderTuple(b);
//     final c1 = ta.codeNums.length.compareTo(tb.codeNums.length);
//     if (c1 != 0) return c1;
//     for (int i = 0; i < ta.codeNums.length; i++) {
//       if (ta.codeNums[i] != tb.codeNums[i])
//         return ta.codeNums[i].compareTo(tb.codeNums[i]);
//     }
//     final c2 = ta.name.compareTo(tb.name);
//     if (c2 != 0) return c2;
//     return ta.id.compareTo(tb.id);
...
//   // ==================== PHÂN LOẠI GROUP ====================
//   List<dynamic> _filterGroups(List<dynamic> all, Set<int> allowedIds) {
//     return all.where((g) => allowedIds.contains(_groupIdOf(g))).toList();
//   }
...
//   // ==================== WIDGET BUILD GROUP (ĐÃ FIX LỖI hasSize) ====================
//   Widget _buildGroupList(List<dynamic> groups, VitalRecordDetailState state) {
//     if (groups.isEmpty) {
...
//         return Column(
//           key: ValueKey('group_${_groupIdOf(g)}'),
//           crossAxisAlignment: CrossAxisAlignment.start,
//           children: [
//             if (g.group?.name != null)
//               Padding(
//                 padding: const EdgeInsets.fromLTRB(8, 16, 8, 8),
//                 child: Text(
//                   _safeStr(g.group?.name),
...
//       child: Column(
//         children: [
//           _buildExpandableSection('Yêu cầu', labGroups, state),
//           _buildExpandableSection('Kết quả', labGroups, state),
//           _buildExpandableSection('Đính kèm', labGroups, state),
//           _buildExpandableSection('Hẹn', labGroups, state),
...
//             final all = List.of(state.groups)..sort(_compareGroups);
//             final normalGroups = _normalGroups(all);
//             final diagnosisGroups = _filterGroups(all, _diagnosisIds);
//             final treatmentGroups = _filterGroups(all, _treatmentIds);
//             final followUpGroups = _filterGroups(all, _followUpIds);

//             return TabBarView(
//               controller: _tabController,
//               children: [
//                 // Tab 1: Thông tin bệnh án
//                 _buildGroupList(normalGroups, state),

//                 // Tab 2: Chuẩn đoán
//                 _buildGroupList(diagnosisGroups, state),

//                 // Tab 3: Xét nghiệm (UI riêng)
//                 _buildLabTab(state, all),

//                 // Tab 4: Điều trị
//                 _buildGroupList(treatmentGroups, state),

//                 // Tab 5: Hẹn
//                 _buildGroupList(followUpGroups, state),
//               ],
//             );
//           },
//         ),
