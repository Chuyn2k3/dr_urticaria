...
/// flatten custom theo chuẩn PHẲNG từ app bệnh nhân:
Map<String, dynamic> flattenCustomFieldValue(
  Map<String, dynamic> value,
  List<CustomFieldGroup>? groups, {
  String prefix = '',
  ...
  for (final group in groups) {
    ...
    for (int i = 0; i < group.fields.length; i++) {
      ...
      final fieldLabel = field.label?.trim() ??
          (groupLabel.isNotEmpty ? groupLabel : 'field_$i');
      ...
      if (fieldValue != null && !_isEmpty(fieldValue)) {
        if (field.type == FieldType.custom &&
            fieldValue is Map<String, dynamic>) {
          result.addAll(flattenCustomFieldValue(fieldValue, field.groups,
              prefix: dotKey));
        } else if (field.type == FieldType.select && fieldValue is Map) {
          final inner = fieldValue[fieldLabel];
          if (inner != null && !_isEmpty(inner)) {
...
class VitalRecordDetailCubit extends Cubit<VitalRecordDetailState> {
  final VitalRecordRepository repo = serviceLocator<VitalRecordRepository>();

  final int medicalRecordId;
  final int templateId;

  VitalRecordDetailCubit({
  ...
  Future<void> load() async {
    ...
    // 1) load template form (groups/indicators)
    try {
      final templateRes = await repo.getTemplate(templateId); // ✅ templateId
      final template = templateRes.data;
      if (template == null) {
        emit(state.copyWith(
          loadingForm: false,
          loadingDetail: false,
          error: 'Không tìm thấy templateId=$templateId',
        ));
        return;
      }

      final futures =
          template.vitalGroupIds.map((id) => repo.getVitalGroup(id));
    ...
    // 2) load detail values (nếu có) rồi merge vào initial maps
    await _loadDetailValues();
  }

  Future<void> _loadDetailValues() async {
    try {
      final detailGroups = await repo.fetchVitalGroups(medicalRecordId);

      final initialValues = <int, dynamic>{};
      final initialNotes = <int, String?>{};

      for (final g in detailGroups) {
        for (final item in g.items) {
          final indicatorId = (item.value?.vitalIndicatorId as int?) ??
              (item.indicator?.id as int?);
          if (indicatorId == null || indicatorId <= 0) continue;

          final v = item.value?.value;
          final n = item.value?.note;

          if (v != null) initialValues[indicatorId] = v;
          if (n != null) initialNotes[indicatorId] = n;
        }
      }
      ...
      // vẫn show form, chỉ báo lỗi detail
      emit(state.copyWith(
        loadingDetail: false,
        error: e.toString(),
      ));
  ...
  void editValue({required int indicatorId, required dynamic newValue}) {
    final map = Map<int, dynamic>.from(state.editedValues);
    map[indicatorId] = newValue;
    emit(state.copyWith(editedValues: map, success: false));
  }

  void editNote({required int indicatorId, String? note}) {
    final map = Map<int, String?>.from(state.editedNotes);
    map[indicatorId] = note;
    emit(state.copyWith(editedNotes: map, success: false));
  }

  VitalIndicator? _findIndicator(int indicatorId) {
    for (final g in state.groups) {
      for (final ind in g.indicators) {
  ...
  Future<void> saveAll() async {
    ...
    try {
      final vitalValues = <Map<String, dynamic>>[];
      final allIds = {...state.editedValues.keys, ...state.editedNotes.keys};

      for (final indicatorId in allIds) {
        final indicator = _findIndicator(indicatorId);
        if (indicator == null) continue;

        final rawValue = state.editedValues.containsKey(indicatorId)
            ? state.editedValues[indicatorId]
            : state.initialValues[indicatorId];

        final rawNote = state.editedNotes.containsKey(indicatorId)
        ...
        if (indicator.valueType == 'custom' &&
          ...
          try {
            if (indicator.valueOptions is List &&
                (indicator.valueOptions as List).isNotEmpty &&
                (indicator.valueOptions as List).first
                    is Map<String, dynamic>) {
              final customField =
                  CustomField.fromJson({'fields': indicator.valueOptions});
              customGroups = customField.groups;
            } else if (indicator.valueOptions is Map<String, dynamic>) {
              final customField = CustomField.fromJson(indicator.valueOptions);
              customGroups = customField.groups;
            }
          } catch (_) {}
        ...
        final hasValue = !_isEmpty(processedValue);
        final hasNote = rawNote != null && rawNote.trim().isNotEmpty;

        // ✅ cho phép update note-only (kể cả value rỗng)
        if (hasValue || hasNote) {
          vitalValues.add({
      ...
      // reload detail values để fill lại
      emit(state.copyWith(loadingDetail: true));
      await _loadDetailValues();
    } catch (e) {
      emit(state.copyWith(saving: false, error: e.toString(), success: false));
    }
  }
}
