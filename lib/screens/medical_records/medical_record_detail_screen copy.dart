...
class MedicalRecordDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        body: TabBarView(
          children: [
            // Tab 1: Tổng quan (Thông tin cơ bản + Bệnh án cấp tính)
            _buildTabTongQuan(),
            // Tab 2: Chẩn đoán
            _buildTabChanDoan(),
            // Tab 3: Xét nghiệm
        ...
        children: [
          _buildInfoCard(
            ...
            [
              ...
              _buildInfoRow('Total IgE (IU/mL)', record.data['totalIgE'] ?? ''),
              _buildInfoRow(
                  'Xét nghiệm khác', record.data['otherLabTests'] ?? ''),
            ],
          ),
          const SizedBox(height: 16),
          if (record.data['rashLocationImages'] != null ||
              record.data['angioedemaLocationImages'] != null)
    ...
    return _buildInfoCard(
      ...
      [
        if (record.data['rashLocationImages'] != null)
          _buildImageSection(
            'Ảnh vị trí nốt đỏ',
            record.data['rashLocationImages'],
          ),
        const SizedBox(height: 16),
        if (record.data['angioedemaLocationImages'] != null)
          _buildImageSection(
            'Ảnh vị trí phù mạch',
            record.data['angioedemaLocationImages'],
          ),
      ],
    );
  }

  Widget _buildImageSection(String title, Map<String, dynamic> images) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
        ...
        children: [
          _buildInfoCard(
            ...
            [
              ...
              _buildInfoRow('Nhập viện', record.data['hospitalization'] ?? ''),
              _buildInfoRow(
                  'Thông tin thuốc đã sử dụng', record.data['drugName'] ?? ''),
              _buildInfoRow('Liều dùng', record.data['drugDosage'] ?? ''),
              _buildInfoRow(
                  'Thời gian dùng', record.data['drugDuration'] ?? ''),
            ],
          ),
        ],
      ),
  ...
  Widget _buildTabHen() {
    return SingleChildScrollView(
      ...
      child: _buildInfoCard(
        ...
        const Color(0xFFF59E0B),
        [
          _buildInfoRow('Ngày hẹn tái khám', record.data['followUpDate'] ?? ''),
          _buildInfoRow(
              'Ghi chú', record.data['followUpNote'] ?? 'Không có ghi chú'),
        ],
      ),
    );
  }
      ...
      child: Column(
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Icon(typeInfo['icon'], color: Colors.white, size: 28),
              const SizedBox(width: 10),
              Icon(statusInfo['icon'], color: Colors.white, size: 28),
            ],
          ),
          const SizedBox(height: 8),
          Text(
  ...
  List<Widget> _buildBasicInfo() {
    return [
      _buildInfoRow('Họ và tên', record.data['fullName'] ?? ''),
      _buildInfoRow('Số CCCD', record.data['nationalId'] ?? ''),
      _buildInfoRow('Tuổi', record.data['age'] ?? ''),
      _buildInfoRow('Giới tính', record.data['gender'] == '0' ? 'Nam' : 'Nữ'),
      _buildInfoRow('Số điện thoại', record.data['phoneNumber'] ?? ''),
      _buildInfoRow('Khu vực sinh sống', record.data['addressArea'] ?? ''),
  ...
  List<Widget> _buildAcuteUrticariaDetails() {
    // Giữ nguyên logic cũ, chỉ trả về danh sách các card cấp tính
    // (Bạn có thể copy nguyên phần _buildAcuteUrticariaDetails() cũ vào đây nếu cần chi tiết hơn)
    return [
      _buildInfoCard('Bệnh án cấp tính - phần chung', Icons.flash_on,
          const Color(0xFFEF4444), [
        _buildInfoRow('Loại tổn thương', record.data['rashOrAngioedema'] ?? ''),
        _buildInfoRow('Lần đầu tổn thương từ bao nhiêu tuần',
            record.data['firstOutbreakSinceWeeks'] ?? ''),
        _buildInfoRow('Số đợt bị mày đay (≥ 2 lần/tuần)',
            record.data['outbreakCount'] ?? ''),
      ]),
      // ... các đợt 1,2,3, hiện tại, thuốc, sẩn phù, phù mạch, yếu tố khởi phát... (copy từ file cũ nếu cần)
    ];
  }
  ...
  Map<String, dynamic> _getTypeInfo(String type) {
    switch (type) {
      ...
      default:
        return {'label': 'Khác', 'color': Colors.grey, 'icon': Icons.help};
    }
  }
